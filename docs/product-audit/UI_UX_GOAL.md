...
Pass6 continued: `ui-ux-pass-06-host-tools.md` covers the consolidated host menu/editor, clear host selection and empty state, pending/failure behavior, a real synthetic accepted host change with departure, and rescheduling with another player's place retained for reconfirmation. Next review recurring cancellation/transfer scope and host permissions, retaining the remaining host-request and editing states.
