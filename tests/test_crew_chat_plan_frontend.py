...
def run(script):
    source = APP[APP.index('  function gameActivityLabel('):APP.index('  function gameResultScoreboardHtml(')]
...
def test_next_session_distinguishes_attendance_and_only_offers_planning_to_joined_groups():
    run('''
      assert.match(crewChatPlanCopy({joined:true,member_count:1},null).title,/Plan with this group/);
      ...
      const game={id:7,scheduled_at:'Friday',court:{name:'Sunset Park'},is_invited:true,visibility:'open'};
      const invited=crewChatPlanCopy({},game);
      assert.equal(invited.gameId,7);
      assert.equal(invited.title,'Pickup session');
      assert.equal(crewChatPlanCopy({},{...game,max_players:2}).title,'Casual singles');
      assert.equal(crewChatPlanCopy({},{...game,max_players:4,game_type:'ranked'}).title,'Ranked doubles');
      assert.equal(invited.detail,'Local Friday · Sunset Park');
...
def test_plan_refresh_preserves_button_nodes_and_uses_text_for_untrusted_names():
    run('''
      ...
      assert.equal(detail.textContent,'Local Friday · <img onerror=bad()>');
