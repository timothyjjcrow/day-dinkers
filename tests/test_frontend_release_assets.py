...
def test_r84_manifest_matches_readable_sources_and_reduces_transfer_size():
    manifest = json.loads((RELEASE / 'manifest.json').read_text())
    assert manifest['release'] == 'r84'
    ...
    # r84 adds emailed sign-in codes, one-tap court conditions and Compete on
    # Me within the caps r83 set (app 305 KiB, aggregate 395 KiB). The
...
def test_release_route_negotiates_precompressed_immutable_assets(filename):
    ...
    plain = client.get(f'/assets/r84/{filename}')
    gzip_response = client.get(
        f'/assets/r84/{filename}', headers={'Accept-Encoding': 'gzip'},
    )
    brotli_response = client.get(
        f'/release-assets/r84/{filename}',
    ...
    assert client.get('/assets/r84/not-generated.js').status_code == 404
    assert client.get('/release-assets/r57/app-v15.min.js').status_code == 404
    assert client.get('/release-assets/r84/app-v15.min.js.map').status_code == 404
    # Older releases remain available to already-open service-worker clients
    # while the document moves them to the new immutable r84 URLs.
...
def test_feature_helpers_are_loaded_before_app_and_available_offline():
    ...
    app_script = '<script defer src="/release-assets/r84/app-v15.min.js"></script>'
    for helper in ('crew-planner-v15', 'tournament-bracket-v15', 'venue-workspace-v15'):
        source = f'/release-assets/r84/{helper}.min.js'
...
def test_vercel_negotiates_committed_brotli_release_assets():
    ...
    for release in ('r58', 'r59', 'r60', 'r61', 'r62', 'r63', 'r64', 'r65', 'r66', 'r67', 'r68', 'r69', 'r70', 'r71', 'r72', 'r73', 'r74', 'r75', 'r76', 'r77', 'r78', 'r79', 'r80', 'r81', 'r82', 'r83', 'r84'):
...
@pytest.mark.parametrize('previous_release', ['r81', 'r82', 'r83'])
@pytest.mark.parametrize('filename', RUNTIME_FILES)
