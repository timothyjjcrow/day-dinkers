...
PUBLIC = ROOT / 'public'
RELEASE = PUBLIC / 'assets' / 'r69'
CI_WORKFLOW = (ROOT / '.github' / 'workflows' / 'backend-ci.yml').read_text()


def test_r69_manifest_matches_readable_sources_and_reduces_transfer_size():
    manifest = json.loads((RELEASE / 'manifest.json').read_text())
    assert manifest['release'] == 'r69'
...
def test_release_route_negotiates_precompressed_immutable_assets():
    ...
    plain = client.get('/assets/r69/app-v15.min.js')
    gzip_response = client.get(
        '/assets/r69/app-v15.min.js', headers={'Accept-Encoding': 'gzip'},
    )
    ...
    assert client.get('/assets/r69/not-generated.js').status_code == 404
    assert client.get('/release-assets/r57/app-v15.min.js').status_code == 404
    assert client.get('/release-assets/r69/app-v15.min.js.map').status_code == 404
    # Older releases remain available to already-open service-worker clients
    # while the document moves them to the new immutable r69 URLs.
...
def test_vercel_negotiates_committed_brotli_release_assets():
    ...
    for release in ('r58', 'r59', 'r60', 'r61', 'r62', 'r63', 'r64', 'r65', 'r66', 'r67', 'r68', 'r69'):
