...
def test_small_activity_sample_is_counted_without_a_popularity_claim(app, client):
    ...
    with app.app_context():
        ...
        # Midday in Chicago, so every visit falls inside the charted hours.
        midday = utcnow().replace(hour=18, minute=0, second=0, microsecond=0)
        for index in range(9):
            at = midday-timedelta(days=index+1)
    ...
    with app.app_context():
        at = utcnow().replace(hour=18, minute=0, second=0, microsecond=0)-timedelta(days=11)
